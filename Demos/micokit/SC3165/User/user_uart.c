...
OSStatus user_uartSend(unsigned char *inBuf, unsigned int inBufLen)
{
  ...
  if( (NULL == inBuf) || ( 0 == inBufLen) ){
    err = kParamErr;
    user_uart_log("[ERR]user_uartSend: user_uartSend input params error!");
    goto exit;
  }
  
//  user_uart_log("[DBG]user_uartSend:[%d]=%.*s", inBufLen, inBufLen, inBuf);
  
  err = MicoUartSend(USER_UART, inBuf, inBufLen);
  require_noerr_action( err, exit, user_uart_log("[ERR]user_uartSend: send to USART error! err=%d", err) );
  return kNoErr;
...
uint32_t user_uartRecv(unsigned char *outBuf, unsigned int getLen)
{
  ...
  if( (NULL == outBuf) || (0 == getLen) ){
    user_uart_log("[ERR]user_uartRecv: user_uartRecv input params error!");
    return 0;
  }
