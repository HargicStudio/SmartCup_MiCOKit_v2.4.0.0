...
static void appNotify_ConnectFailedHandler(OSStatus err, mico_Context_t * const inContext)
{
  ...
  if(NULL != app_context_global){
    ...
    // if wifi connected false, clear up wifi parameter and reset system
    err_internal = mico_system_context_restore( inContext );
    app_log("[DBG]appNotify_ConnectFailedHandler: system context restore with err code %d", err_internal);
  
    err_internal = mico_system_power_perform( inContext, eState_Software_Reset );
    app_log("[DBG]appNotify_ConnectFailedHandler: system power perform with err code %d", err_internal);
  }
}
